...
function calcularDuracion(lat1, lon1, lat2, lon2) {
    ...
    // Tiempo de rodaje + despegue + aterrizaje (30 min fijos)
    const tiempoTierra = 0.5;

    const horas = (distancia / velocidad) + tiempoTierra;

    // Mínimo 30 minutos
    return Math.max(horas, 0.5).toFixed(1);
}
...
function mostrarMapa() {
  ...
  document.getElementById("duracionVuelo").innerHTML =
  "Duración aproximada del vuelo: " + formatearDuracion(parseFloat(duracion));
...
try {
  ...
  const response = await fetch(
    "/itinerarios?idUsuario=" + idUsuario
  );
  ...
  try {
    const res = await fetch("/itinerarios", {
      method: "POST",
